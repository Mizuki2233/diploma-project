	...
	<div class="addEdit-block" :style='{"padding":"30px","backgroundSize":" 100% 100%","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20220730/cb2195540cde42b7abf14af25c15da3e.png) no-repeat  center bottom / cover","display":"flex","height":"calc(100vh - 120px)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="归还说明" prop="guihaishuoming">
					<el-input
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.guihaishuoming" label="归还说明" prop="guihaishuoming">
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `cheliangguihai/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.cheliangguihaiCrossAddOrUpdateFlag = false;
						 this.parent.search();
