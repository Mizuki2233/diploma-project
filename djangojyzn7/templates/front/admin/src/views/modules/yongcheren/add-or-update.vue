	...
	<div class="addEdit-block" :style='{"padding":"30px","backgroundSize":" 100% 100%","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20220730/cb2195540cde42b7abf14af25c15da3e.png) no-repeat  center bottom / cover","display":"flex","height":"calc(100vh - 120px)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="upload" v-if="type!='info' && !ro.shenfenzhengzhao" label="身份证照" prop="shenfenzhengzhao">
					<file-upload
						tip="点击上传身份证照"
						action="file/upload"
						:limit="3"
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="upload" v-else-if="ruleForm.shenfenzhengzhao" label="身份证照" prop="shenfenzhengzhao">
					...
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.shenfenzhengzhao.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="upload" v-if="type!='info' && !ro.jiazhao" label="驾照" prop="jiazhao">
					<file-upload
						tip="点击上传驾照"
						action="file/upload"
						:limit="3"
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="upload" v-else-if="ruleForm.jiazhao" label="驾照" prop="jiazhao">
					...
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.jiazhao.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
			</template>
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='shenfenzhengzhao'){
							this.ruleForm.shenfenzhengzhao = obj[o];
							this.ro.shenfenzhengzhao = true;
							continue;
						}
						if(o=='jiazhao'){
							this.ruleForm.jiazhao = obj[o];
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yongcheren/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yongcherenCrossAddOrUpdateFlag = false;
						 this.parent.search();
